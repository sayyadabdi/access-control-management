...
--algorithm PermissionManager
{
    ...
    macro incompleteRevoke(app) { appPerms[app] := [i \in Permissions |-> NULL]; }
    
    procedure ask(app, perm) {l1: if(appPerms[app][perm] = GRANT) { skip; }; return}
    procedure installApp(app) {l2: installed[app] := TRUE; return}
    procedure uninstallApp(app) {l3: incompleteRevoke(app); installed[app] := FALSE; return}
    procedure makeDecision(app, perm)
    {
        l4: either { call ask(app, perm); }
            or { arbitraryDecision(app, perm); };
        l5: return;
    }
...
*************)             
\* BEGIN TRANSLATION (chksum(pcal) = "b8bf02ef" /\ chksum(tla) = "2bd5e678")
...
l4(self) == /\ pc[self] = "l4"
            ...
            /\ UNCHANGED << permConsents, installed, app_i, app_u, app, perm >>
...
l5(self) == /\ pc[self] = "l5"
            ...
            /\ stack' = [stack EXCEPT ![self] = Tail(stack[self])]
            /\ UNCHANGED << appPerms, permConsents, installed, app_, perm_, 
                            app_i, app_u >>

makeDecision(self) == l4(self) \/ l5(self)
...
\* Last modified Sun Feb 26 22:04:05 IRST 2023 by Amirhosein
