...
UriPermModeFlag == {FLAG_GRANT_READ_URI_PERMISSION, FLAG_GRANT_WRITE_URI_PERMISSION}
...
--algorithm PermissionManager
{
    variables CP = [a \in Applications |-> NULL];
              ...
              manifest = [i \in Applications |-> NULL];
    ...
    fair process (a \in Applications)
    {
        PLATFORM:- while (TRUE)
        {
                ...
                or { either { call uninstallApp(self); }
                     or
                     {
                        ...
                        or
                        {
                            ...
                            or { with (a \in Applications \ {self}) { with (u \in Uris) { with (p \in Permissions) { call grantUriPermission(a, u, FLAG_GRANT_WRITE_URI_PERMISSION, p); } } } }
                        }
                     }
                   }
            }
            ...
            {
                with (u \in Uris) { manifest[self] := u; };
...
VARIABLES CP, installed, appPerms, permsInUse, cpConsent, appPermConsents, 
          ...
          a1, a2, app_un, app_s, perm_, app, uri, mode, perm
...
ASK_CP_FROM_USER(self) == /\ pc[self] = "ASK_CP_FROM_USER"
                          ...
                          /\ UNCHANGED << CP, installed, appPerms, permsInUse, 
                                          appPermConsents, manifest, app_, 
                                          app_d, app_u, a1, a2, app_un, app_s, 
                                          perm_, app, uri, mode, perm >>
...
RETURNING(self) == /\ pc[self] = "RETURNING"
                   ...
                   /\ UNCHANGED << CP, installed, appPerms, permsInUse, 
                                   ...
                                   app_un, app_s, perm_, app, uri, mode, perm >>
                    ...
                    /\ IF appCustomPerms[a1[self]][a2[self]] = GRANT
                          ...
                          ELSE /\ IF appCustomPerms[a1[self]][a2[self]] = DENY
                                     ...
                                     ELSE /\ IF CP[a2[self]] = NORMAL
                                          ...
                                          /\ UNCHANGED << a1, a2 >>
                    /\ UNCHANGED << CP, installed, appPerms, permsInUse, 
                                    cpConsent, appPermConsents, manifest, app_, 
                                    app_d, app_u, app_un, app_s, perm_, app, 
                                    uri, mode, perm >>
...
UNINSTALL_APP(self) == /\ pc[self] = "UNINSTALL_APP"
                       ...
                       /\ UNCHANGED << CP, appPerms, cpConsent, appCustomPerms, 
                                       ...
                                       a1, a2, app_s, perm_, app, uri, mode, 
                                       perm >>
...
CHECKING_PERMISSION_URI(self) == /\ pc[self] = "CHECKING_PERMISSION_URI"
                                 /\ IF manifest[app[self]] # uri[self]
                                       THEN /\ pc' = [pc EXCEPT ![self] = Head(stack[self]).pc]
                                            ...
                                            /\ stack' = [stack EXCEPT ![self] = Tail(stack[self])]
                                       ELSE /\ pc' = [pc EXCEPT ![self] = "ASK_PERMISSION"]
                                            /\ UNCHANGED << stack, app, uri, 
                                                            mode, perm >>
                                 /\ UNCHANGED << CP, installed, appPerms, 
                                                 permsInUse, cpConsent, 
                                                 appPermConsents, 
                                                 appCustomPerms, manifest, 
                                                 app_, app_d, a1_, a2_, app_u, 
...
grantUriPermission(self) == CHECKING_PERMISSION_URI(self)
                               \/ ASK_PERMISSION(self)
                               \/ MAKE_DECISION(self) \/ USING_PERM(self)
...
PLATFORM(self) == /\ pc[self] = "PLATFORM"
                  /\ IF installed[self] = TRUE
                        THEN /\ \/ /\ /\ app_u' = [app_u EXCEPT ![self] = self]
                                   ...
                                   /\ UNCHANGED <<app_d, a1, a2, app_un, app, uri, mode, perm>>
                                \/ /\ \/ /\ /\ app_un' = [app_un EXCEPT ![self] = self]
                                         ...
                                         /\ UNCHANGED <<app_d, a1, a2, app, uri, mode, perm>>
                                      \/ /\ \/ /\ IF CP[self] = NULL
                                                     ...
                                                     ELSE /\ pc' = [pc EXCEPT ![self] = "PLATFORM"]
                                                          /\ UNCHANGED << stack, 
                                                                          app_d >>
                                               /\ UNCHANGED <<a1, a2, app, uri, mode, perm>>
                                            \/ /\ \/ /\ \E application \in (Applications \ {self}):
                                                     ...
                                                     /\ UNCHANGED <<app, uri, mode, perm>>
                                                  \/ /\ \E a \in Applications \ {self}:
                                                          \E u \in Uris:
                                                            \E p \in Permissions:
                                                              /\ /\ app' = [app EXCEPT ![self] = a]
                                                                 /\ mode' = [mode EXCEPT ![self] = FLAG_GRANT_WRITE_URI_PERMISSION]
                                                              ...
                                                              /\ pc' = [pc EXCEPT ![self] = "CHECKING_PERMISSION_URI"]
                                                     /\ UNCHANGED <<a1, a2>>
                                               /\ app_d' = app_d
                                         /\ UNCHANGED app_un
                                   /\ app_u' = app_u
                             /\ UNCHANGED << manifest, app_ >>
                        ELSE /\ \E u \in Uris:
...
TypeOK == /\ appCustomPerms \in [Applications -> [Applications -> PermissionRequestDecision]]
          ...
          /\ manifest \in [Applications -> Uris \cup { NULL }]
...
\* Last modified Thu Mar 23 17:08:15 GMT+03:30 2023 by Amirhosein
