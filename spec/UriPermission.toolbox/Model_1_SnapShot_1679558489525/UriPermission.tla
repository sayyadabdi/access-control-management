...
--algorithm PermissionManager
{
    ...
    fair process (a \in Apps)
    {
        PLATFORM:- while (TRUE)
        {
            ...
            {
                either { appPerms[self] := [p \in Perms |-> NULL]; call uninstallApp(self); }
...
*************)             
\* BEGIN TRANSLATION (chksum(pcal) = "d704ad12" /\ chksum(tla) = "9af5d0b1")
...
PLATFORM(self) == /\ pc[self] = "PLATFORM"
                  /\ IF installed[self] = TRUE
                        THEN /\ \/ /\ appPerms' = [appPerms EXCEPT ![self] = [p \in Perms |-> NULL]]
...
\* Last modified Thu Mar 23 11:31:24 GMT+03:30 2023 by Amirhosein
